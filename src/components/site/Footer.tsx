...
export function Footer() {
  return (
    <footer className="bg-ink text-white pt-12 sm:pt-16 pb-8">
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-10">
          <div className="lg:col-span-4">
            <div className="inline-flex items-center rounded-2xl bg-white px-4 py-3 shadow-sm">
              <img src={logo} alt="Praveen J & Associates" className="h-9 sm:h-11 w-auto object-contain" />
            </div>
            <p className="mt-4 text-xs sm:text-sm leading-relaxed text-white/70 max-w-sm">
            ...
            <div className="mt-5 flex gap-2">
              {[Linkedin, Instagram, Facebook, Youtube].map((Icon, i) => (
                <a key={i} href="#" aria-label="Social media" className="h-9 w-9 rounded-full bg-white/10 hover:bg-brand flex items-center justify-center transition-colors">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 sm:grid-cols-4 gap-6">
            {([
              ...
              <div key={col.h}>
                <h4 className="font-display font-semibold text-sm sm:text-base mb-3">{col.h}</h4>
                <ul className="space-y-2 text-xs sm:text-sm text-white/70">
                  {col.items.map(i => (
                    <li key={i.label}>
                      <Link to={i.to} className="hover:text-white transition-colors block truncate">{i.label}</Link>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="mt-8 sm:mt-10 pt-6 border-t border-white/10 grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs sm:text-sm text-white/80">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="truncate">pjain4069@gmail.com</span>
          </div>
          <div className="flex items-start sm:items-center gap-2 sm:justify-end">
            <MapPin className="h-4 w-4 text-brand shrink-0 mt-0.5 sm:mt-0" />
            <span className="leading-relaxed text-xs sm:text-sm">
              First Floor, Office no. 19, Evershine Mall, New Link Rd, Malad West, Mumbai 400064.
            </span>
          </div>
        </div>
        <div className="mt-6 sm:mt-8 text-center text-[11px] sm:text-xs text-white/50">
