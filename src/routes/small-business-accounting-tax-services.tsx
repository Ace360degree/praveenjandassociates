...
export const Route = createFileRoute("/small-business-accounting-tax-services")({
  loader: () => {
    const s = getAccountingServiceBySlug("small-business-accounting-tax-services");
    if (!s) throw notFound();
    return s;
  },
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `/small-business-accounting-tax-services` },
      ],
      links: [{ rel: "canonical", href: `/small-business-accounting-tax-services` }],
      scripts: [
        {
...
function Hero({ s }: { s: NonNullable<ReturnType<typeof getAccountingServiceBySlug>> }) {
...
function FinalCTA({ s }: { s: NonNullable<ReturnType<typeof getAccountingServiceBySlug>> }) {
  return (
    <section className="py-16 bg-brand text-white">
      <div className="container mx-auto px-4 text-center max-w-3xl">
        <h2 className="font-display text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold break-words">Simplify your business finances and stay compliant with expert support.</h2>
...
function Related({ currentSlug }: { currentSlug: string }) {
  const related = ACCOUNTING_SERVICES.filter((s) => s.slug !== currentSlug).slice(0, 6);
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
