...
import { CONSULTATION_SERVICES, getConsultationServiceBySlug } from "@/data/consultationServices";

export const Route = createFileRoute("/consultation/$slug")({
  loader: ({ params }) => {
    const service = getConsultationServiceBySlug(params.slug);
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `/consultation/${s.slug}` },
      ],
      links: [{ rel: "canonical", href: `/consultation/${s.slug}` }],
      scripts: [
        {
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-24 text-center">
        ...
        <p className="text-muted-foreground mt-3">Looks like this Consultation service page doesn't exist.</p>
        <Link to="/consultation" className="inline-flex mt-6 items-center gap-2 text-brand font-semibold">
          <ArrowRight className="h-4 w-4" /> Back to Consultation services
        </Link>
      </main>
      <Footer />
    </div>
  ...
  component: ConsultationServicePage,
});
...
function Hero({ s }: { s: NonNullable<ReturnType<typeof getConsultationServiceBySlug>> }) {
...
function FinalCTA({ s }: { s: NonNullable<ReturnType<typeof getConsultationServiceBySlug>> }) {
  return (
...
function RelatedServices({ currentSlug }: { currentSlug: string }) {
  const related = CONSULTATION_SERVICES.filter((s) => s.slug !== currentSlug).slice(0, 6);
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
        <h2 className="font-display text-2xl md:text-3xl font-bold text-ink text-center">Explore More Consultation Services</h2>
        ...
        <div className="text-center mt-8">
          <Link to="/consultation" className="inline-flex items-center gap-2 text-brand font-semibold hover:underline">
            View all Consultation services <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
