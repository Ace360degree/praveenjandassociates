  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `/income-tax/${s.slug}` },
      ],
      links: [{ rel: "canonical", href: `/income-tax/${s.slug}` }],
      scripts: [
        {
...
function Hero({ s }: { s: NonNullable<ReturnType<typeof getItrServiceBySlug>> }) {
...
function RelatedServices({ currentSlug }: { currentSlug: string }) {
  const related = ITR_SERVICES.filter((s) => s.slug !== currentSlug).slice(0, 6);
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
