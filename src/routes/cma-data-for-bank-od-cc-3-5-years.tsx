...
import { BANK_LOAN_SERVICES, getBankLoanServiceBySlug } from "@/data/bankLoanServices";

export const Route = createFileRoute("/cma-data-for-bank-od-cc-3-5-years")({
  loader: () => {
    const s = getBankLoanServiceBySlug("cma-data-for-bank-od-cc-3-5-years");
    if (!s) throw notFound();
    return s;
  },
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `/cma-data-for-bank-od-cc-3-5-years` },
      ],
      links: [{ rel: "canonical", href: `/cma-data-for-bank-od-cc-3-5-years` }],
      scripts: [
        {
  ...
  notFoundComponent: () => (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-24 text-center">
        ...
        <p className="text-muted-foreground mt-3">This accounting service page doesn't exist.</p>
        <Link to="/accounting-audit-services" className="inline-flex mt-6 items-center gap-2 text-brand font-semibold">
          <ArrowRight className="h-4 w-4" /> Back to Accounting & Audit services
        </Link>
      </main>
      <Footer />
    </div>
  ...
  return (
    <nav className="border-b bg-muted/30" aria-label="Breadcrumb">
      <ol className="container mx-auto px-4 py-3 text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 text-muted-foreground flex-wrap max-w-full overflow-hidden">
        ...
        <li><Link to="/accounting-audit-services" className="hover:text-brand">Accounting & Audit</Link></li>
        <ChevronRight className="h-3 w-3" />
...
function Hero({ s }: { s: NonNullable<ReturnType<typeof getBankLoanServiceBySlug>> }) {
      ...
      <div className="container mx-auto px-4 py-12 lg:py-20 grid lg:grid-cols-2 gap-10 items-center">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-brand/10 text-brand px-3 py-1 text-xs font-semibold">
            <Sparkles className="h-3.5 w-3.5" /> Accounting & Audit · {s.title}
          </span>
          <h1 className="mt-4 font-display text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold leading-tight text-ink break-words">{s.h1}</h1>
  ...
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
        <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-ink break-words text-center">Common Problems Faced</h2>
...
function WhoFor({ items }: { items: string[] }) {
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
        <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-ink break-words text-center">Who Should Use This Service?</h2>
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
          {items.map((w) => (
    ...
    <section className="py-14 bg-muted/30">
      <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 max-w-5xl">
        <div className="bg-white rounded-2xl p-6 lg:p-8 border shadow-sm">
          <h3 className="font-display text-xl font-bold text-ink flex items-center gap-2"><Sparkles className="h-5 w-5 text-brand" /> Benefits</h3>
          <ul className="mt-4 space-y-2.5">
            {items.map((b) => (
              <li key={b} className="flex gap-2 items-start text-sm"><CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0 mt-0.5" /><span>{b}</span></li>
            ))}
          </ul>
        </div>
        <div className="bg-white rounded-2xl p-6 lg:p-8 border shadow-sm">
          <h3 className="font-display text-xl font-bold text-ink flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-brand" /> Important Points</h3>
          <ul className="mt-4 space-y-2.5">
            {important.map((b) => (
...
function FinalCTA({ s }: { s: NonNullable<ReturnType<typeof getBankLoanServiceBySlug>> }) {
  return (
    <section className="py-16 bg-brand text-white">
      <div className="container mx-auto px-4 text-center max-w-3xl">
        <h2 className="font-display text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold break-words">Improve your chances of loan approval with professionally prepared CMA data.</h2>
...
function Related({ currentSlug }: { currentSlug: string }) {
  const related = BANK_LOAN_SERVICES.filter((s) => s.slug !== currentSlug).slice(0, 6);
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4">
        <h2 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-ink break-words text-center">Explore More Accounting & Audit Services</h2>
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
          {related.map((r) => (
            <Link key={r.slug} to="/accounting-audit-services/$slug" params={{ slug: r.slug }} className="group p-5 rounded-xl border hover:border-brand/40 hover:shadow-lg transition-all bg-white">
